...
export class UsuariosService extends TercerosService {
   ...
   listHistory( id: number ) {
      return this.authHttp.get( this.usuariosServiceURL + "auditoria/Usuarios/" + id ).map( ( res: Response ) => res.json() as VHistoricoUsuario[] );
   }
   ...
   createUser( p: Usuario ): Promise<Usuario> {
      return this.authHttp.post( this.usuariosServiceURL, JSON.stringify( p ) ).toPromise().then( res => res.json() as Usuario ).catch( this.handleError );
   }
   ...
   updatePass( c: Usuario ) {
      return this.authHttp.put( this.usuariosServiceURL + "cambiarPass/" + c.contrasenaAntigua + '/', JSON.stringify( c ) ).toPromise().then( res => {
         if ( res.json() == true ) {
            return true;
         } else {
            return false;
   ...
   createUserGroup( p: UsuarioGrupoGestion ): Promise<UsuarioGrupoGestion> {
      ...
      return this.authHttp.post( this.usuariosGruposServiceURL, JSON.stringify( p ) ).toPromise().then( res => res.json() as UsuarioGrupoGestion ).catch( this.handleError );
   }
   ...
   createUserRole( p: UsuarioRol ): Promise<UsuarioRol> {
      ...
      return this.authHttp.post( this.usuariosRolesServiceURL, JSON.stringify( p ) ).toPromise().then( res => res.json() as UsuarioRol ).catch( this.handleError );
   }
   
   readUserGroups( p: number ) {
      return this.authHttp.get( this.usuariosGruposServiceURL + "vista/" + p ).map( ( res: Response ) => res.json() as VUsuarioGrupoGestion[] );
   }
