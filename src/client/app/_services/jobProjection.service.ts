...
export class JobProjectionService {
   ...
   getLisStructure( id: number ) {
      return this.authHttp.get( this.serviceURL + 'estructuraOrganizacional/buscarTipo/' + id )
      .map( ( res: Response ) => res.json() as OrganizationalStructure[] );
   }

   getLisStructurePositions( id: number ) {
      return this.authHttp.get( this.serviceURL + 'estructuraOrganizacionalCargos/buscarCargo/' + id )
      .map( ( res: Response ) => res.json() as OrganizationalStructurePositions[] );
   }

   getListJobProjctionByArea( id: number ) {
      return this.authHttp.get( this.serviceURL + 'proyeccionesLaborales/buscarArea/' + id )
      .map( ( res: Response ) => res.json() as JobProjection[] );
   }
